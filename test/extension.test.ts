...
test('the ui extension owns extensions.ui, builds the kit from the project files and serves hashed assets at its mount', async () => {
    ...
    assert.ok(ui.active);
    const kit = ui.kit;
    assert.equal(kit.assetsBase, '/assets/ui/static');
    ...
    assert.equal((await instance.handle(request(`/assets/ui/static/${css.name}`, 'GET', { 'if-none-match': `"${css.hash}"` }))).status, 304);
    assert.equal((await instance.handle(request(`/assets/ui/static/${css.name}`, 'HEAD'))).body, undefined);
    assert.equal((await instance.handle(request('/assets/ui/static/other.css'))).status, 404);
    assert.equal((await instance.handle(request(`/assets/ui/static/${css.name}`, 'POST'))).status, 405);
    assert.equal((await instance.handle(request(`/assets/ui/${css.name}`))).status, 404, 'assets live only under the immutable prefix');
...
test('the asset handler answers 405 with an allow header for other methods and 304 only on a matching etag', async () => {
    ...
    const css = ui.kit.assets[0]!, path = `/assets/ui/static/${css.name}`;
    ...
    assert.equal((await instance.handle(request('/assets/ui/static/missing.css', 'POST'))).status, 405, 'method is checked before the path');
...
test('the registration declares the immutable asset prefix and asset responses satisfy the runtime cache exception', async () => {
    const ui = createUiExtension({ projectSha256: sha, projectRoot: await project() });
    assert.deepEqual(ui.registration.immutableAssets, { prefix: '/static' });
    assert.equal(uiAssetPrefix, '/static');
    const instance = await ui.registration.activate({}, activation(['/assets/ui']));
    for (const asset of ui.kit.assets) {
        const path = `/assets/ui/static/${asset.name}`;
        assert.ok(path.startsWith('/assets/ui/static/'), 'served under <mount><prefix>/');
        for (const [method, extra] of [['GET', {}], ['HEAD', {}], ['GET', { 'if-none-match': `"${asset.hash}"` }]] as const) {
            const result = await instance.handle(request(path, method, { cookie: 'session=x', authorization: 'Bearer y', ...extra }));
            assert.ok(result.status === 200 || result.status === 304, `${method} ${result.status}`);
            const etags = result.headers.filter(([name]) => name.toLowerCase() === 'etag').map(([, value]) => value);
            assert.deepEqual(etags, [`"${asset.hash}"`], 'exactly one strong etag');
            assert.equal(result.headers.some(([name]) => /^(set-cookie|vary|cache-control)$/i.test(name)), false, 'no set-cookie, vary or cache-control');
        }
    }
    await instance.close?.();
});
